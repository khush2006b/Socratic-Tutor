...
Graph topology:
  START → build_context → generate_response → parse_tags → persist_data → END
...
def _build_tutor_graph():
    ...
    g.add_edge("parse_tags",        "persist_data")
    g.add_edge("persist_data",       END)

    compiled = g.compile()
    logger.info("Tutor graph compiled — nodes: build_context → generate_response → parse_tags → persist_data")
    return compiled
